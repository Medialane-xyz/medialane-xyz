...
import { ArrowLeft, Heart, Share2, Copy, Flag, ShoppingCart, HandshakeIcon, GitBranch, Zap, Eye, Loader2 } from "lucide-react"
    ...
    return (
        <div className="min-h-screen bg-background pt-20">
            <div
                ...
                style={{
                    backgroundImage: `linear-gradient(135deg, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.4) 100%), url('${asset.image}')`,
            ...
            <div className="container mx-auto px-3 md:px-6 py-6 md:py-8 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                            ...
                            <div className="relative aspect-square">
                                ...
                                <div className="absolute bottom-4 left-4 flex gap-2">
                                    ...
                                    <div className="flex items-center gap-1.5 bg-black/60 backdrop-blur px-3 py-1.5 rounded-full">
                                        ...
                                        <span className="text-white text-xs font-medium">{asset.views}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-5 lg:max-h-fit">
                                ...
                                <div className="space-y-3 pt-3">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-xs text-muted-foreground text-center px-1">
                                            {isOwner ? "List your asset on the marketplace" : "Acquire complete property rights and ownership"}
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-2 pt-2 border-t border-white/10 bg-purple-500/10 rounded-lg p-3 -mx-6 px-3">
                                        ...
                                        <p className="text-xs text-muted-foreground text-center px-1">
                                            Create a derivative work with new license. Price auto-calculated with {asset.royalty}% royalty to creator.
                                        </p>
                                        <div className="bg-purple-500/20 rounded p-2 mt-2">
                                            <p className="text-xs text-purple-200">
                                                <span className="font-semibold">💡 Monetization:</span> Earn from your creations while supporting original creators
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <Card className="border-white/10 bg-muted/30">
                                <CardContent className="p-4 text-center">
                                    <div className="font-bold text-xl">{asset.holders || 1}</div>
                ...
                {/* Creator card */}
                <Card className="border-white/10 bg-gradient-to-br from-muted/40 to-muted/20 backdrop-blur mt-8">
                    <CardContent className="p-5 md:p-6">
                            ...
                            <div className="flex items-center gap-4 flex-1 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                                        Creator {asset.verified && <span className="text-blue-500">Verified</span>}
                                    </p>
                                </div>
                            </div>
                        </div>
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={() => router.push(`/creators/${asset.ownerAddress}`)}
                ...
                <Tabs defaultValue="details" className="w-full mt-8">
                    <TabsList className="grid w-full grid-cols-3 mb-6 bg-muted/50 h-11 md:h-10">
                        ...
                        <TabsTrigger value="lineage" className="text-xs md:text-sm">Provenance</TabsTrigger>
                    ...
                    <TabsContent value="details" className="space-y-6">
                        <Card>
                            <CardContent className="space-y-6 pt-6">
                                <div>
                                    <h4 className="font-semibold mb-4 text-sm md:text-base">Asset Information</h4>
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-start">
                                            <span className="text-muted-foreground text-sm">Token ID</span>
                                            <span className="font-mono text-xs md:text-sm text-right truncate ml-2">{asset.tokenId}</span>
                                        </div>
                                        <div className="flex justify-between items-start">
                                            <span className="text-muted-foreground text-sm">Collection</span>
                                            <span className="text-sm text-right truncate ml-2">{asset.collectionName}</span>
                                        </div>
                                        {asset.collectionAddress && (
                                            <div className="flex justify-between items-start">
                                                <span className="text-muted-foreground text-sm">Contract</span>
                                                <span className="font-mono text-xs text-right truncate ml-2" title={asset.collectionAddress}>
                                                    {asset.collectionAddress.slice(0, 10)}...{asset.collectionAddress.slice(-8)}
                                                </span>
                                            </div>
                                        )}
                                        <div className="flex justify-between items-start">
                                            <span className="text-muted-foreground text-sm">Blockchain</span>
                                            <span className="text-sm text-right">{asset.blockchain}</span>
                                        </div>
                                        <div className="flex justify-between items-start">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            {asset.attributes.map((attr: any, index: number) => (
                                                <div key={index} className="p-3 md:p-4 bg-muted/30 rounded-lg border border-white/10">
                                                    <div className="text-xs text-muted-foreground">{attr.trait_type || "Attribute"}</div>
                            ...
                            <CardContent className="py-6">
                                <div className="flex items-center gap-4 mb-4">
                                    {asset.collectionImage && (
                                        <Image src={asset.collectionImage} alt={asset.collectionName} width={64} height={64} className="rounded-lg" />
                                    )}
                                    <div>
                                        <h4 className="font-semibold text-lg">{asset.collectionName}</h4>
                                        {asset.collectionAddress && (
                                            <p className="text-xs text-muted-foreground font-mono">{asset.collectionAddress.slice(0, 16)}...</p>
                                        )}
                                    </div>
                                </div>
                                {asset.collectionAddress && (
                                    <Button variant="outline" onClick={() => router.push(`/collections/${asset.collectionAddress}`)} className="w-full">
                                        View Collection
                                    </Button>
                                )}
                            </CardContent>
